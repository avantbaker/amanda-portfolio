...
export const min = (pixels: number) => (first: any, ...args: any) => css`
	@media ${minWidthQuery(pixels)} {
		${css(first, ...args)};
	}
`;

const max = (pixels: number) => (first: any, ...args: any) => css`
	@media ${maxWidthQuery(pixels)} {
		${css(first, ...args)};
	}
`;

const heightMin = (pixels: number) => (first: any, ...args: any) => css`
	@media ${minHeightQuery(pixels)} {
		${css(first, ...args)};
	}
`;

const heightMax = (pixels: number) => (first: any, ...args: any) => css`
